...
local BanOrder = {
	"Ban5Min","Ban15Min","Ban30Min","Ban1Hour","Ban1Day","Ban30Day","Ban365Day"
}
	...
	if Toggled then
		local function Handle(Target)
			...
			table.insert(Connections,
				Target:GetAttributeChangedSignal("IsBanned"):Connect(function()
					if not Target:GetAttribute("IsBanned") then
						Ban(Target)
					end
				end)
		...
		table.insert(Connections, Players.PlayerAdded:Connect(Handle))
	else
...
local UnbanTimes = {
	[300]="Unban5Min",[900]="Unban15Min",[1800]="Unban30Min",
	[3600]="Unban1Hour",[86400]="Unban1Day",
	[2592000]="Unban30Day",[31536000]="Unban365Day"
}

local function GetCategory(Time)
	local Closest, Cat = math.huge, nil
	for T, C in pairs(UnbanTimes) do
		if Time <= T and T < Closest then
			Closest, Cat = T, C
		end
	end
	return Cat
...
local function Unban()
	local Ends = LocalPlayer:GetAttribute("BanEndsAt")
	if not Ends then return end
	local Cat = GetCategory(Ends - os.time())
	if not Cat then return end

	if UnbanInventory[Cat].Value < 1 then
		BuyUnban:FireServer(Cat)
	end

	repeat
		UnbanRequest:FireServer(Cat)
		task.wait()
...
DarkEye.addModule("Player", "AutoUnban", function(Toggled)
	if Toggled then
		...
		Thread:Maid("DE_Unban",
			LocalPlayer:GetAttributeChangedSignal("IsBanned"):Connect(function()
				if LocalPlayer:GetAttribute("IsBanned") then
					Unban()
				end
			end)
...
TextChatService.OnIncomingMessage = function(Message)
	...
	if Message.Status == Enum.TextChatMessageStatus.Sending then
		if Text:sub(1,1) == ";" and Sender == LocalPlayer then
